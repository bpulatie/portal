...
public class async_job_Services : System.Web.Services.WebService
{
    DataLayer.async_job oData = new DataLayer.async_job();
    SPA.spaResponse myResponse = new SPA.spaResponse();
    DataLayer.sys_session oSession = new DataLayer.sys_session();

    public async_job_Services()
    {
    }
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = false, ResponseFormat = ResponseFormat.Json)]
    public void GetByID(string job_id)
    {
        try
        {
            myResponse.data = oData.GetByID(job_id);
            myResponse.result = true;
            myResponse.message = "OK";
        }
        catch (Exception ex)
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = false, ResponseFormat = ResponseFormat.Json)]
    public void GetAllParametersByJob(string job_id, int pageNo, int rows)
    {
        try
        {
            string filter = SPA.spaGlobals.ProcessFilters(this.Context.Request);

            myResponse.data = oData.GetAllParametersByJob(job_id, filter, pageNo, rows);
            myResponse.result = true;
            myResponse.message = "OK";
        }
        catch (Exception ex)
