...
public class inv_item_Services : System.Web.Services.WebService
{
    SPA.spaResponse myResponse = new SPA.spaResponse();
    DataLayer.sys_session oSession = new DataLayer.sys_session();

    DataLayer.inv_item oData = new DataLayer.inv_item();

    public inv_item_Services()
    {
    }
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = false, ResponseFormat = ResponseFormat.Json)]
    public void GetByID(string item_id)
    {
        try
        {
            myResponse.data = oData.GetByID(item_id);
            myResponse.result = true;
            myResponse.message = "OK";
        }
        catch (Exception ex)
