...
public class inv_category_level_Services : System.Web.Services.WebService
{
    ...
    public inv_category_level_Services()
    {
    }
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = false, ResponseFormat = ResponseFormat.Json)]
    public void GetByLevel(string level, string id, int pageNo, int rows)
    {
        try
        {
            myResponse.data = oData.GetByLevel(oSession.client_id, level, id);
            myResponse.result = true;
            myResponse.message = "OK";
        }
        catch (Exception ex)
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = false, ResponseFormat = ResponseFormat.Json)]
    public void GetByID(string category_level_id)
    {
        try
        {
            myResponse.data = oData.GetByID(category_level_id);
            myResponse.result = true;
            myResponse.message = "OK";
        }
        catch (Exception ex)
