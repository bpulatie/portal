...
public class sys_site_Services : System.Web.Services.WebService
{
    SPA.spaResponse myResponse = new SPA.spaResponse();
    DataLayer.sys_session oSession = new DataLayer.sys_session();

    DataLayer.sys_site oData = new DataLayer.sys_site();

    public sys_site_Services()
    {
    }
    ...
    public void GetAllByUserAssigned(string user_id, int pageNo, int rows)
    {
        try
        {
            ...
            myResponse.data = oData.GetAllByUserAssigned(oSession.client_id, user_id, filter, pageNo, rows);
            myResponse.result = true;
            myResponse.message = "OK";
        }
        catch (Exception ex)
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = false, ResponseFormat = ResponseFormat.Json)]
    public void GetAllByUserUnassigned(string user_id, int pageNo, int rows)
    {
        try
        {
            string filter = SPA.spaGlobals.ProcessFilters(this.Context.Request);

            myResponse.data = oData.GetAllByUserUnassigned(oSession.client_id, user_id, filter, pageNo, rows);
            myResponse.result = true;
            myResponse.message = "OK";
        }
        catch (Exception ex)
